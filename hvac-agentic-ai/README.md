...
- `POST /chat/stream` — same routing logic, but streamed as Server-Sent Events:
  one event per `agent_start` / `token` / `tool_call` / `tool_result` /
  `step_start` / `step_done` / `agent_done` / `pipeline_done`, so a client can
  show which LLM is executing which task in real time instead of waiting
  silently for the ~2 minute full pipeline run. This is what the UI actually uses.
- `POST /pipeline/{unit_id}` / `POST /pipeline/{unit_id}/stream` — same pair,
  forcing a specific unit through the pipeline instead of parsing it from a message.
...
- **Streaming** (`agents/base_llm_agent.py`'s `run_streaming`, `agents/llm_client.py`'s
  `stream_chat_completion`) requests `stream_options={"include_usage": True}` so
  the context monitor still gets real token counts even in streaming mode — vLLM
  needs to be reasonably current to honor this; if your version ignores it, usage
  will silently stay at 0 for streamed calls (non-streaming `run()` is unaffected).
